...
const groupingEngine_1 = require("./groupingEngine");
...
async function optimizeRoute(params) {
    ...
    if (!origin || !destination)
        return { stops: [], totalDistanceKm: 0, provider: 'mock' };
    if (config_1.USE_LIVE_GOOGLE_MAPS && API_KEY && middle.length > 0) {
...
async function optimizeWaypoints(waypoints) {
...
async function optimizeWithGoogle(origin, middle, destination) {
    try {
        ...
        if (!route?.waypoint_order || !route.legs)
            return null;
...
function optimizeWithNearestNeighbour(origin, middle, destination) {
    const remaining = [...middle];
    const ordered = [origin];
    while (remaining.length > 0) {
        const current = ordered[ordered.length - 1];
        const nearestIndex = remaining.reduce((winner, waypoint, index) => (0, groupingEngine_1.haversine)(current.lat, current.lng, waypoint.lat, waypoint.lng) < (0, groupingEngine_1.haversine)(current.lat, current.lng, remaining[winner].lat, remaining[winner].lng) ? index : winner, 0);
        ordered.push(remaining.splice(nearestIndex, 1)[0]);
    }
    ordered.push(destination);
    return buildResult(ordered, [], 'mock');
}
function buildResult(ordered, liveLegDistances, provider, googleWaypointOrder) {
    let cumulative = 0;
    const stops = ordered.map((waypoint, index) => {
        const legDistance = index === 0 ? 0 : liveLegDistances[index - 1] || (0, groupingEngine_1.haversine)(ordered[index - 1].lat, ordered[index - 1].lng, waypoint.lat, waypoint.lng);
