...
function RequestView({ pickup, setPickup, dropoff, setDropoff, vehicle, setVehicle, when, setWhen, prebookTime, setPrebookTime, query, setQuery, destinations: filtered, mapPin, setMapPin, onBack, onRequest, fare }: { pickup: Location; setPickup: (value: Location) => void; dropoff: Location; setDropoff: (value: Location) => void; vehicle: Vehicle; setVehicle: (value: Vehicle) => void; when: 'now' | 'later'; setWhen: (value: 'now' | 'later') => void; prebookTime: string; setPrebookTime: (value: string) => void; query: string; setQuery: (value: string) => void; destinations: Location[]; mapPin: { x: number; y: number }; setMapPin: (value: { x: number; y: number }) => void; onBack: () => void; onRequest: () => void; fare: number }) { 
  ...
  useEffect(() => {
    if (!livePickupQuery) { setLivePickupResults(pickupPresets); return }
    const t = setTimeout(() => {
...
function PoolView({ pickup, dropoff, vehicle, fare, paid, setPaid, chatOpen, setChatOpen, messages, message, setMessage, sendMessage, onTrack }: { pickup: Location; dropoff: Location; vehicle: Vehicle; fare: number; paid: boolean; setPaid: (value: boolean) => void; chatOpen: boolean; setChatOpen: (value: boolean) => void; messages: { name: string; text: string; time: string }[]; message: string; setMessage: (value: string) => void; sendMessage: () => void; onTrack: () => void }) {
  ...
  const poolIdPrefix = activePool?.id ? activePool.id.replace(/-/g, '').substring(0, 4).toUpperCase() : '4821'
  const displayMembers = realMembers.length > 0 ? realMembers.map(m => ({
  ...
  return <div className="pool-view"><div className="pool-top"><div><p className="eyebrow">POOL FOUND · #CP{poolIdPrefix}</p><h2>Your ride crew is ready</h2></div><div className="matched-chip"><span /><strong>{displayMembers.length}/{vehicle === 'CAB_4' ? 4 : 3}</strong><small>Matched</small></div></div><div className="route-card"><div className="route-summary"><div><span className="route-point navy" /><strong>{pickup.name}</strong></div><span className="route-time">ETA: 5m</span><div className="route-rule" /><div><span className="route-point gold" /><strong>{dropoff.name}</strong></div><span className="route-time">25m trip</span></div>
  <div style={{ width: '100%', height: '140px', marginTop: '16px', borderRadius: '12px', overflow: 'hidden' }}>
    <InteractiveMap pickup={pickup} dropoff={dropoff} onMapClick={() => {}} />
  </div>
  </div><div className="pool-info-row"><span><Users size={17} /> {vehicle === 'AUTO_3' ? 'Auto' : 'Cab'} · {vehicle === 'CAB_4' ? 4 : 3} seats</span><span><Clock3 size={17} /> 25 min</span><span><Navigation size={17} /> {dropoff.name.includes('Airport') ? '12.8 km' : '6.4 km'}</span></div><section className="member-card"><div className="card-title"><h3>Your crew</h3><button onClick={() => setChatOpen(true)}><MessageCircle size={16} /> Chat</button></div>{displayMembers.map((member) => <div className="member-row" key={member.name}><span className={`member-avatar ${member.color}`}>{member.initials}</span><div><strong>{member.name} {member.userId === localStorage.getItem('token') && '(You)'}</strong><small>Stop {member.stop} · {member.stop === 1 ? pickup.name : member.stop === 3 ? 'GH' : 'Main Gate'}</small></div><span className={member.paid || paid ? 'paid-status' : 'pending-status'}>{member.paid || paid ? <><Check size={13} /> Paid</> : 'Pending'}</span></div>)}</section><section className="payment-card"><div className="payment-top"><div><p className="eyebrow">YOUR ESTIMATED SHARE</p><h2>₹{paid ? 68 : fare}</h2></div><span className="split-badge"><WalletCards size={15} /> Distance split</span></div><div className="payment-bar"><span style={{ width: paid ? '100%' : '66%' }} /></div><div className="payment-foot"><span>{paid ? 'All members paid' : 'Waiting for others to pay'}</span><span>Fare may vary ±₹8</span></div>{!paid ? <button className="primary-button wide" onClick={() => setPaid(true)}><CreditCard size={17} /> Pay with Razorpay · ₹{fare}</button> : <button className="success-button wide" onClick={onTrack}><Check size={17} /> All paid · View live ride</button>}</section>{chatOpen && <ChatDrawer messages={messages} message={message} setMessage={setMessage} sendMessage={sendMessage} onClose={() => setChatOpen(false)} />}</div> }
